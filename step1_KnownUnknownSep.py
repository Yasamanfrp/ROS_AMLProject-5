...
def _do_epoch(args,feature_extractor,rot_cls,obj_cls,source_loader,optimizer,device):
    ...
    running_corrects_cls= 0
    running_corrects_rot=0

    # pass images and labels through the training 
    for it, (data, class_l, data_rot, rot_l) in enumerate(source_loader):
       ...
       #soft max or Argmax?
        ...
        # Update corrects
        running_corrects_cls += torch.sum(cls_pred == class_l)#.data.item()
        running_corrects_rot += torch.sum(rot_pred == rot_l)#.data.item()


    
    j = len(source_loader.dataset) #total number of images in source domain
    #Apply accuracy equation
    acc_cls = running_corrects_cls/float(j)
    acc_rot = running_corrects_rot/float(j)
